...
public class ChatServer extends UnicastRemoteObject implements ChatServerIF {
    ...
    public void updateChat(String name, String nextPost) throws RemoteException {
        ...
        String message = "(" + time + ") " + name + " : " + nextPost + "\n";
        sendToAll(message);
    }
    
    //Send an ImageIcon to all connected clients 
    public void updateChatIcon(String name, ImageIcon nextPost) throws RemoteException {
        LocalTime localTime = LocalTime.now();
        var time = localTime.toString();
        time = time.substring(0, 5);
        String message = "(" + time + ") " + name + " : ";
        sendToAll(message);
        sendIconToAll(nextPost);
        message = "\n";
        sendToAll(message);
    }
    ...
    //Send a emoji to all users
    public void sendIconToAll(ImageIcon newEmoji) {
        for (Chatter c : chatters) {
            try {
                c.getClient().iconFromServer(newEmoji);
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    public void sendIconPM(int[] privateGroup, String privateMessage, ImageIcon emoji) throws RemoteException {
        Chatter pc;
        for (int i : privateGroup) {
            pc = chatters.elementAt(i);
            pc.getClient().messageFromServer(privateMessage);
            pc.getClient().iconFromServer(emoji);
            String temp = "\n";
            pc.getClient().messageFromServer(temp);
        }
    }
